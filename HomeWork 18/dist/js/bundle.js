...
function calc() {
  ...
  var persons = document.querySelectorAll('.counter-block-input')[0],
      //поле Количество людей
  restDays = document.querySelectorAll('.counter-block-input')[1],
      //поле На сколько дней
  place = document.getElementById('select'),
      //выбор базы отдыха
  totalValue = document.getElementById('total'),
      //ощая сумма
  personsSum = 0,
      //переменная для Количество людей
  daysSum = 0,
      //переменная Количество дней
  total = 0; //переменная общей суммы

  totalValue.innerHTML = 0; //зануляем сумму на сайте

  persons.addEventListener('change', function () {
    //при вводе в поле Количество людей считается сумма, с проверка ненулевого второго поля
  ...
  restDays.addEventListener('change', function () {
    //при вводе в поле На сколько дней считается сумма, с проверка ненулевого второго поля
    daysSum = +this.value;
    total = (personsSum + daysSum) * 4000;
  ...
  place.addEventListener('change', function () {
    //изменение суммы при выборе базы
    if (restDays.value == '' || persons.value == '') {
      //если какое то из полей пустые - занулять сумму
      totalValue.innerHTML = 0;
    } else {
      var a = total; //сумма, но она идет в качестве переменной, так что каждый раз будет такая же
...
function form() {
  ...
  form.addEventListener('submit', function (event) {
    //при нажатии на отправить - форма отправляется
    ...
    function smartRequest() {
      //описываем функцию "умного" запроса через промисы
      return new _Promise(function (resolve, reject) {
        var request = new XMLHttpRequest(); //начинаем описывать и отправлять запрос
  ...
  contactForm.addEventListener('submit', function (event) {
    ...
    function smartRequestContact() {
      ...
      return new _Promise(function (resolve, reject) {
        ...
        request.open('POST', 'server.php');
        request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
        var formData = new FormData(contactForm); //создаем объект formData
        //превращаем formData в JSON-объект
...
function modal() {
  var more = document.querySelector('.more'),
      //получаем форму "узнать больше"
  overlay = document.querySelector('.overlay'),
      //получаем анимацию открытия
  close = document.querySelector('.popup-close'); //получаем крестик "закрыть"

  more.addEventListener('click', function () {
    //при нажатии на кнопку "Унать больше"
  ...
  close.addEventListener('click', function () {
    //при нажатии на крестик сверху справа в форме
  ...
  var descriptionbtn = document.querySelectorAll('.description-btn'); //массив кнопок "узнать подробнее"
...
function slider() {
  ...
  dotsWrap = document.querySelector('.slider-dots'),
      //обёртка точек слайдера(тот раздел, где точки)
  ...
  function showSlides(n) {
    if (n > slides.length) {
      //чтобы перематывалось в начало, если дошло до конца
      slideIndex = 1;
    }

    if (n < 1) {
      //чтобы перематывалось в конец, если нажали стрелку НАЗАД, когда стоит первый слайд
    ...
    }); //убираем "активность" с любых точек
  ...
  prev.addEventListener('click', function () {
    //при нажатии на стрелку НАЗАД - показ прдыдущего слайда
    plusSlides(-1);
  });
  next.addEventListener('click', function () {
    //при нажатии на стрелку ВПЕРЕД - показ следующего слайда
    plusSlides(1);
  });
  dotsWrap.addEventListener('click', function (event) {
    //при клике на точке - показывается соответствующий слайдер
    for (var i = 0; i < dots.length + 1; i++) {
      //перебираем все слайдеры
      if (event.target.classList.contains('dot') && event.target == dots[i - 1]) {
        //делегирование, i-1, потому что  slideIndex д.б. от 1 до 4, это номер слайда, а не индекс массива
...
function tabs() {
  var tab = document.querySelectorAll('.info-header-tab'),
      //все табы с .info-header-tab
  info = document.querySelector('.info-header'),
      //элемент-родитель .info-header
  tabContent = document.querySelectorAll('.info-tabcontent'); //тексты к табам .info-tabcontent

  function hideTabContent(a) {
    //функция, скрывающая контент табов(для начала)
  ...
  info.addEventListener('click', function (event) {
    //при клике на определенный таб - показываем определенный контент ЕСЛИ ПРОЩЕ, то мы определяем НОМЕР ткнутого таба, скрываем все контенты, а затем показываем только нужный[i-ый] контент, совпадающий по номеру с ткнутым табом
    var target = event.target;

    if (target && target.classList.contains('info-header-tab')) {
      //если ткнули в элемент(существующий) и этот элемент ТОТ
      for (var i = 0; i < tab.length; i++) {
        //итерируем по всем табам
        if (target == tab[i]) {
          //к кликнутому пользователем элементу подобрали через итерацию нужный таб: 
...
function timer() {
  //Timer
  var deadline = '2019-12-31 07:00';

  function getTimeRemainig(endtime) {
    var t = Date.parse(endtime) - Date.parse(new Date()),
        //сколько осталось до дедлайна
    seconds = Math.floor(t / 1000 % 60),
        //получаем количество целых секунд
    minutes = Math.floor(t / 1000 / 60 % 60),
        //получаем количество целых минут
    ...
    return {
      //возвращаем объект с данными об оставшемся до дедлайна времени
      'total': t,
  ...
  function setClock(id, endtime) {
    //получаем данные на HTML странице
    ...
    function updateClock() {
      ...
      var t = getTimeRemainig(endtime); //записываем в t весь объект с временными параметрами

      days.textContent = t.days; //обновляем дни на странице, 0 в начале ставить НЕ надо

      if (t.hours >= 10) {
        //если количество часов больше или равно 10
        hours.textContent = t.hours; //то вставляем в блок без изменений
      } else {
        //если 9 и меньше(т.е. одна цифра)
...
__webpack_require__(/*! nodelist-foreach-polyfill */ "./node_modules/nodelist-foreach-polyfill/index.js"); //подключаю полифил для forEach 


__webpack_require__(/*! formdata-polyfill */ "./node_modules/formdata-polyfill/formdata.min.js"); //полифил formData


window.addEventListener('DOMContentLoaded', function () {
  //после загрузки DOM-дерева назначаем событие
  "use strict";
